...
const LayoutBase = props => {
  const {
    ...
    categoryOptions,
    currentCategory,
    posts // LayoutBase 也需要访问所有 posts 来进行搜索过滤
  } = props
      ...
      >
        ...
        {isSearchOpen && (
            <div className="search-modal-overlay">
                ...
                {/* >>>>>>> 优化2: 搜索结果区域，传递 searchResults 给 LayoutPostList <<<<<<< */}
                <div className="search-input-results">
                    {/* 只有当 filterKey 不为空时才显示搜索结果，否则显示空 */}
                    <LayoutPostList posts={filterKey ? searchResults : []} isSearchResult={true} />
                </div>
            </div>
        )}
      </div>
...
const LayoutPostList = props => {
  ...
  // >>>>>>> 优化3: 移除 getRandomRotation 函数，恢复垂直摆放 <<<<<<<
              ...
              <div className='books-on-shelf'>
                {row.map(post => (
                  <div 
                    ...
                    // >>>>>>> 优化4: 移除 style={{ transform: getRandomRotation() }} <<<<<<<
